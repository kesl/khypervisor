...
/* 
 * Register guest ID for irq injection.
 * This function will be called in VGICD.
 */
hvmm_status_t add_virqmap_table(uint32_t virq, vmid_t vmid)
{
    if (_virqmap[virq].virq == 0) { // Entry is empty
        _virqmap[virq].virq = virq;
        _virqmap[virq].vmid = vmid;
        return HVMM_STATUS_SUCCESS;
    }
