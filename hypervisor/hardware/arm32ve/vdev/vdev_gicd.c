...
static struct vdev_memory_map _vdev_gicd_info = { .base = CFG_GIC_BASE_PA
		| GIC_OFFSET_GICD, .size = 4096, };
...
static hvmm_status_t vdev_gicd_reset_values(void) {
	...
	for (i = 0; i < NUM_GUESTS_STATIC; i++) {
		 ...
		 */
		int j = 0;
		uint32_t up_address = 0x00000000;
//		for (j = 0; j < 40; j++)
//			_regs[i].ITARGETSR[j] = 0;

		_regs[i].CTLR = (uint32_t)(*((volatile unsigned int*)0x10481000));
		_regs[i].TYPER = (uint32_t)(*((volatile unsigned int*)0x10481004));
		_regs[i].IIDR = (uint32_t)(*((volatile unsigned int*)0x10481008));
		printH("vdev init:'%s' vmid:%d, gicd TYPER:%x\n", __func__, i, _regs[i].TYPER);
		printH("vdev init:'%s' vmid:%d, gicd IIDR:%x\n", __func__, i, _regs[i].IIDR);
		// init
        for(j = 0; j < VGICD_NUM_IGROUPR; j++) {
		    _regs[i].IGROUPR[j] = (uint32_t)(*((volatile unsigned int*)0x10481000 + up_address));
		    up_address +=  0x00000004;
        }

        up_address =  0x00000000;
        for(j = 0; j < VGICE_NUM_ISENABLER; j++) {
		    _regs[i].ISENABLER[j] = (uint32_t)(*((volatile unsigned int*)0x10481100 + up_address));
		    up_address +=  0x00000004;
        }

        up_address =  0x00000000;
        for(j = 0; j < VGICE_NUM_ICENABLER; j++) {
		    _regs[i].ICENABLER[j] = (uint32_t)(*((volatile unsigned int*)0x10481180 + up_address));
		    up_address +=  0x00000004;
        }

        up_address =  0x00000000;
        for(j = 0; j < VGICE_NUM_ISPENDR; j++) {
		    _regs[i].ISPENDR[j] = (uint32_t)(*((volatile unsigned int*)0x10481200 + up_address));
		    up_address +=  0x00000004;
        }

        up_address =  0x00000000;
        for(j = 0; j < VGICE_NUM_ICPENDR; j++) {
		    _regs[i].ICPENDR[j] = (uint32_t)(*((volatile unsigned int*)0x10481280 + up_address));
		    up_address +=  0x00000004;
        }

        up_address =  0x00000000;
        for(j = 0; j < VGICE_NUM_ISACTIVER; j++) {
		    _regs[i].ISACTIVER[j] = (uint32_t)(*((volatile unsigned int*)0x10481300 + up_address));
		    up_address +=  0x00000004;
        }

        up_address =  0x00000000;
        for(j = 0; j < VGICE_NUM_ICACTIVER; j++) {
		    _regs[i].ICACTIVER[j] = (uint32_t)(*((volatile unsigned int*)0x10481300 + up_address));
		    up_address +=  0x00000004;
        }

        up_address =  0x00000000;
        for(j = 0; j < VGICE_NUM_IPRIORITYR; j++) {
		    _regs[i].IPRIORITYR[j] = (uint32_t)(*((volatile unsigned int*)0x10481400 + up_address));
		    up_address +=  0x00000004;
        }

        up_address =  0x00000000;
        for(j = 0; j < VGICE_NUM_ITARGETSR; j++) {
		    _regs[i].ITARGETSR[j] = (uint32_t)(*((volatile unsigned int*)0x10481800 + up_address));
		    up_address +=  0x00000004;
        }

        up_address =  0x00000000;
        for(j = 0; j < VGICE_NUM_ICFGR; j++) {
		    _regs[i].ICFGR[j] = (uint32_t)(*((volatile unsigned int*)0x10481C00 + up_address));
		    up_address +=  0x00000004;
        }

        _regs[i].PPISR = (uint32_t)(*((volatile unsigned int*)0x10481D00));

        up_address =  0x00000000;
        for(j = 0; j < VGICE_NUM_SPISR; j++) {
		    _regs[i].SPISR[j] = (uint32_t)(*((volatile unsigned int*)0x10481D04 + up_address));
		    up_address +=  0x00000004;
        }

        _regs[i].SGIR = (uint32_t)(*((volatile unsigned int*)0x10481F00));

        up_address =  0x00000000;
        for(j = 0; j < VGICE_NUM_CPENDSGIR; j++) {
		    _regs[i].CPENDSGIR[j] = (uint32_t)(*((volatile unsigned int*)0x10481F10 + up_address));
		    up_address +=  0x00000004;
        }

        up_address =  0x00000000;
        for(j = 0; j < VGICE_NUM_SPENDSGIR; j++) {
		    _regs[i].SPENDSGIR[j] = (uint32_t)(*((volatile unsigned int*)0x10481F20 + up_address));
		    up_address +=  0x00000004;
        }
/*
...
struct vdev_ops _vdev_gicd_ops = { .init = vdev_gicd_reset_values, .check =
		vdev_gicd_check, .read = vdev_gicd_read, .write = vdev_gicd_write,
		.post = vdev_gicd_post, };

struct vdev_module _vdev_gicd_module = { .name =
		"K-Hypervisor vDevice GICD Module", .author = "Kookmin Univ.", .ops =
		&_vdev_gicd_ops, };
