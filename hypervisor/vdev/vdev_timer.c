...
static struct vdev_info _vdev_timer_info = {
...
static int32_t vdev_vtimer_read(struct arch_vdev_info *info,
...
static int32_t vdev_vtimer_write(struct arch_vdev_info *info,
                        struct arch_regs *regs)
{
    uint32_t offset = info->fipa - _vdev_timer_info.base;
...
static int32_t vdev_vtimer_post(struct arch_vdev_info *info,
                        struct arch_regs *regs)
{
    uint8_t isize = 4;
...
static int32_t vdev_vtimer_check(struct arch_vdev_info *info,
                        struct arch_regs *regs)
{
    uint32_t offset = info->fipa - _vdev_timer_info.base;
