...
void pl011_putc(const char c)
{
    ...
    while (*((uint32_t *)(PL011_BASE + 0x18)) & 0x20)
        ;
    /* Send the character */
    volatile char *pUART = (char *) PL011_BASE;
    *pUART = c;
}
...
char pl011_getc(uint32_t base)
{
    ...
    if (data & 0xFFFFFF00) {
        /* Clear the error */
        volatile uint32_t *pUART = (uint32_t *) (PL011_BASE + PL011_UARTECR);
        *pUART = 0xFFFFFFFF;
        return -1;
    }
...
void pl011_init(uint32_t base, uint32_t baudrate,
